...
class Prediction:
    ...
    def bootstrapper(self, preds_with_sigmoid, targets, index_list, testsetname):
        ...
        # 95% confidence interval for AUC (only for average)
        sorted_AUC = AUC_list.copy()
        sorted_AUC = sorted_AUC.mean(1)
        sorted_AUC.sort()
        auc_95_ci_lower = sorted_AUC[int(0.025 * 1000)]
        auc_95_ci_upper = sorted_AUC[int(0.975 * 1000)]

        sorted_accuracy = accuracy_list.copy()
        sorted_accuracy = sorted_accuracy.mean(1)
        sorted_accuracy.sort()
        accuracy_95_ci_lower = sorted_accuracy[int(0.025 * 1000)]
        accuracy_95_ci_upper = sorted_accuracy[int(0.975 * 1000)]

        sorted_specificity = specificity_list.copy()
        sorted_specificity = sorted_specificity.mean(1)
        sorted_specificity.sort()
        specificity_95_ci_lower = sorted_specificity[int(0.025 * 1000)]
        specificity_95_ci_upper = sorted_specificity[int(0.975 * 1000)]

        sorted_sensitivity = sensitivity_list.copy()
        sorted_sensitivity = sorted_sensitivity.mean(1)
        sorted_sensitivity.sort()
        sensitivity_95_ci_lower = sorted_sensitivity[int(0.025 * 1000)]
        sensitivity_95_ci_upper = sorted_sensitivity[int(0.975 * 1000)]
        ...
        print(f'\t avg AUROC: {AUC_list.mean() * 100:.2f} ± {AUC_list.std() * 100:.2f}% | avg accuracy: {accuracy_list.mean() * 100:.2f} ± {accuracy_list.std() * 100:.2f}%'
              ...
              f' | avg sensitivity (recall): {sensitivity_list.mean() * 100:.2f}% ± {sensitivity_list.std() * 100:.2f}% | avg F1: {F1_list.mean() * 100:.2f} ± {F1_list.std() * 100:.2f}%\n')
        ...
        msg = f'\n\n----------------------------------------------------------------------------------------\n' \
              ...
              f'avg AUROC: {AUC_list.mean() * 100:.2f} ± {AUC_list.std() * 100:.2f}% [95% CI: {auc_95_ci_lower * 100:.2f}, {auc_95_ci_upper * 100:.2f}] | avg accuracy: {accuracy_list.mean() * 100:.2f} ± {accuracy_list.std() * 100:.2f}% [95% CI: [{accuracy_95_ci_lower * 100:.2f}, {accuracy_95_ci_upper * 100:.2f}] ' \
              f' | avg specificity: {specificity_list.mean() * 100:.2f} ± {specificity_list.std() * 100:.2f}% [95% CI: {specificity_95_ci_lower * 100:.2f}, {specificity_95_ci_upper * 100:.2f}]' \
              f' | avg sensitivity (recall): {sensitivity_list.mean() * 100:.2f} ± {sensitivity_list.std() * 100:.2f}% [95% CI: {sensitivity_95_ci_lower * 100:.2f}, {sensitivity_95_ci_upper * 100:.2f}] | avg F1: {F1_list.mean() * 100:.2f} ± {F1_list.std() * 100:.2f}%\n\n'
        ...
        for idx, pathology in enumerate(self.label_names):
            sorted_AUC = AUC_list.copy()
            sorted_AUC = sorted_AUC[:, idx]
            sorted_AUC.sort()
            auc_95_ci_lower = sorted_AUC[int(0.025 * 1000)]
            auc_95_ci_upper = sorted_AUC[int(0.975 * 1000)]

            msg = f'{pathology}: {AUC_list[:, idx].mean() * 100:.2f} ± {AUC_list[:, idx].std() * 100:.2f}% [95% CI: {auc_95_ci_lower * 100:.2f}, {auc_95_ci_upper * 100:.2f}] | '
            with open(os.path.join(self.params['target_dir'], self.params['stat_log_path']) + '/Test_on_' + str(testsetname), 'a') as f:
                f.write(msg)
        ...
        for idx, pathology in enumerate(self.label_names):
            sorted_accuracy = accuracy_list.copy()
            sorted_accuracy = sorted_accuracy[:, idx]
            sorted_accuracy.sort()
            accuracy_95_ci_lower = sorted_accuracy[int(0.025 * 1000)]
            accuracy_95_ci_upper = sorted_accuracy[int(0.975 * 1000)]

            msg = f'{pathology}: {accuracy_list[:, idx].mean() * 100:.2f} ± {accuracy_list[:, idx].std() * 100:.2f}% [95% CI: [{accuracy_95_ci_lower * 100:.2f}, {accuracy_95_ci_upper * 100:.2f}] | '
            with open(os.path.join(self.params['target_dir'], self.params['stat_log_path']) + '/Test_on_' + str(testsetname), 'a') as f:
                f.write(msg)
        ...
        for idx, pathology in enumerate(self.label_names):
            sorted_specificity = specificity_list.copy()
            sorted_specificity = sorted_specificity[:, idx]
            sorted_specificity.sort()
            specificity_95_ci_lower = sorted_specificity[int(0.025 * 1000)]
            specificity_95_ci_upper = sorted_specificity[int(0.975 * 1000)]

            msg = f'{pathology}: {specificity_list[:, idx].mean() * 100:.2f} ± {specificity_list[:, idx].std() * 100:.2f}% [95% CI: {specificity_95_ci_lower * 100:.2f}, {specificity_95_ci_upper * 100:.2f}] | '
            with open(os.path.join(self.params['target_dir'], self.params['stat_log_path']) + '/Test_on_' + str(testsetname), 'a') as f:
                f.write(msg)
        ...
        for idx, pathology in enumerate(self.label_names):
            sorted_sensitivity = sensitivity_list.copy()
            sorted_sensitivity = sorted_sensitivity[:, idx]
            sorted_sensitivity.sort()
            sensitivity_95_ci_lower = sorted_sensitivity[int(0.025 * 1000)]
            sensitivity_95_ci_upper = sorted_sensitivity[int(0.975 * 1000)]

            msg = f'{pathology}: {sensitivity_list[:, idx].mean() * 100:.2f} ± {sensitivity_list[:, idx].std() * 100:.2f}% [95% CI: {sensitivity_95_ci_lower * 100:.2f}, {sensitivity_95_ci_upper * 100:.2f}] | '
            with open(os.path.join(self.params['target_dir'], self.params['stat_log_path']) + '/Test_on_' + str(testsetname), 'a') as f:
                f.write(msg)
